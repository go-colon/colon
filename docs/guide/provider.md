...
colon框架使用ServiceProvider机制来满足协议，通过service Provder提供某个协议服务的具体实现。

这样如果开发者对具体的实现协议的服务类的具体实现不满意，则可以很方便的通过切换具体协议的Service Provider来进行具体服务的切换。
...
我们可以使用命令 `./colon provider new` 来创建一个新的service provider
...
create provider success, folder path: ~/go-colon/colon/demo/app/provider/test
...
colon 框架任务，作为一个业务的服务提供者，定义一个好的协议是最重要的事情。
...
import (
	"github.com/go-colon/colon/framework"
)
...
colon的路由，controller的定义是选择基于gin框架进行扩展的。

所有的gin框架的路由，参数获取，验证，context都和gin框架是相同的。

唯一不同的是gin的全局路由gin.Engine实现了colon的容器结构，可以对gin.Engine进行服务提供的注入，且可以从context中获取具体的服务。
...
colon 框架默认自带了一些服务提供者，提供基础的服务接口协议，可以通过 `./colon provider list` 来获取已经安装的服务提供者。

```
[~/Documents/workspace/colon_workspace/demo5]$ ./colon provider list
colon:app
colon:env
colon:config
colon:log
...
colon 框架自带的服务提供者的 key 是以 `colon:` 开头。目的为的是与自定义服务提供者的 key 区别开。

colon 框架自带的服务提供者具体定义的协议可以参考：[provider](/provider/)
