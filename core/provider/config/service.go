...
// ColonConfig  表示colon框架的配置文件服务
type ColonConfig struct {
	c        core.Container         // 容器
	folder   string                 // 文件夹
...
// 读取某个配置文件
func (conf *ColonConfig) loadConfigFile(folder string, file string) error {
	conf.lock.Lock()
...
// 删除文件的操作
func (conf *ColonConfig) removeConfigFile(folder string, file string) error {
	conf.lock.Lock()
	defer conf.lock.Unlock()
	s := strings.Split(file, ".")
	// 只有yaml或者yml后缀才执行
...
// NewColonConfig 初始化Config方法
func NewColonConfig(params ...interface{}) (interface{}, error) {
	container := params[0].(core.Container)
	envFolder := params[1].(string)
	envMaps := params[2].(map[string]string)

	// 实例化
	colonConf := &ColonConfig{
		c:        container,
		folder:   envFolder,
		envMaps:  envMaps,
	...
	for _, file := range files {
		fileName := file.Name()
		err := colonConf.loadConfigFile(envFolder, fileName)
		if err != nil {
			log.Println(err)
			continue
		}
			...
			case ev := <-watch.Events:
				{
					...
					if ev.Op&fsnotify.Create == fsnotify.Create {
						...
						colonConf.loadConfigFile(folder, fileName)
					}
					...
					if ev.Op&fsnotify.Remove == fsnotify.Remove {
						...
						colonConf.removeConfigFile(folder, fileName)
					}
				}
...
// 通过path来获取某个配置项
func (conf *ColonConfig) find(key string) interface{} {
	conf.lock.RLock()
