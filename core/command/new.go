...
var newCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		}
		{
			...
			prompt := &survey.Input{
				Message: "请输入版本名称(参考 https://github.com/go-colon/colon/releases，默认为最新版本)：",
			}
			...
			if version != "" {
				// 确认版本是否正确
				release, _, err = client.Repositories.GetReleaseByTag(context.Background(), "gocolon", "colon", version)
				if err != nil || release == nil {
					fmt.Println("版本不存在，创建应用失败，请参考 https://github.com/go-colon/colon/releases")
					return nil
				}
			}
			if version == "" {
				release, _, err = client.Repositories.GetLatestRelease(context.Background(), "gocolon", "colon")
		...
		fmt.Println("colon框架版本：", release.GetTagName())

		templateFolder := filepath.Join(currentPath, "template-colon-"+version+"-"+cast.ToString(time.Now().Unix()))
		...
		// 获取folder下的gocolon-colon-xxx相关解压目录
		...
		for _, fInfo := range fInfos {
			// 找到解压后的文件夹
			if fInfo.IsDir() && strings.Contains(fInfo.Name(), "gocolon-colon-") {
		...
		// 删除framework 目录
		os.RemoveAll(path.Join(folder, "core"))
		...
		filepath.Walk(folder, func(path string, info os.FileInfo, err error) error {
			...
			if path == filepath.Join(folder, "go.mod") {
				...
				c = bytes.ReplaceAll(c, []byte("module github.com/go-colon/colon"), []byte("module "+mod))
				c = bytes.ReplaceAll(c, []byte("require ("), []byte("require (\n\tgithub.com/go-colon/colon "+version))
			...
			isContain := bytes.Contains(c, []byte("github.com/go-colon/colon/app"))
			if isContain {
				fmt.Println("更新文件:" + path)
				c = bytes.ReplaceAll(c, []byte("github.com/go-colon/colon/app"), []byte(mod+"/app"))
				err = ioutil.WriteFile(path, c, 0644)
				if err != nil {
					return err
				}
