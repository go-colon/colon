...
// 初始化配置文件
func initDevConfig(c core.Container) *devConfig {
	// 设置默认值
	devConfig := &devConfig{
		Port: "8087",
		Backend: struct {
...
// NewProxy 初始化一个Proxy
func NewProxy(c core.Container) *Proxy {
...
func (p *Proxy) restartBackend() error {
	...
	colonAddress := fmt.Sprintf(":" + port)

	bin := os.Args[0]
	// 使用命令行启动后端进程
	cmd := exec.Command(bin, "app", "start", "--address="+colonAddress)
