...
var deployRollbackCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		if len(args) != 2 {
			return errors.New("参数错误,请按照参数进行回滚 ./colon deploy rollback [version] [frontend/backend/all]")
		}
...
func deployBuildBackend(c *cobra.Command, deployFolder string) error {
	...
	if err != nil {
		log.Fatalln("colon go: 请在Path路径中先安装go")
	}
...
func deployUploadAction(deployFolder string, container core.Container, end string) error {
	configService := container.MustMake(contract.ConfigKey).(contract.Config)
...
// 上传部署文件夹
func uploadFolderToSFTP(container core.Container, localFolder, remoteFolder string, client *sftp.Client) error {
	logger := container.MustMake(contract.LogKey).(contract.Log)
	// 遍历本地文件
...
// 创建部署的folder
func createDeployFolder(c core.Container) (string, error) {
